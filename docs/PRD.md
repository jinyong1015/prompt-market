...
- **as-is:**
  ...
  - 버튼: `[상세보기]` → `/prompt/[id]`, `[장바구니 담기]` / 비활성 `[장바구니에 있음]`(중복 불가, 토스트 `이미 장바구니에 있습니다`), 구매 시 `[구매완료]`.
  - 장바구니 담기 성공 토스트: `장바구니에 담았습니다`. 비로그인 시 `로그인이 필요합니다`(리다이렉트 없음).
  - 검색/필터/정렬/페이지네이션 없음.
- **to-be:**
...
- **as-is:**
  ...
  - **미구매:**
    ...
    - `[장바구니 담기]` / `[장바구니에 있음]`(중복 불가), `[바로 구매하기]`(결제 다이얼로그). 이미 구매한 상품은 재구매 불가.
  - **구매 완료:**
    - `구매 완료` 표시.
...
- **as-is:**
  ...
  - **상품 있음:** 이미지·제목·가격, `[삭제]`(X), 즉시 목록 갱신.
  - 동일 상품 중복 담기 불가(ID 단위 1개). 재담기 시 토스트 `이미 장바구니에 있습니다`.
  - 이미 구매한 상품은 담기·재구매 불가.
  - 결제 요약: `총 상품 금액`, `결제 예정 금액`.
  - `[결제하기]` → 토스페이먼츠 **데모 시뮬레이션** 다이얼로그.
  - 수량/쿠폰 없음.
- **to-be:**
  - 실 토스페이먼츠 결제창 + 서버 금액 검증.
  - 결제 성공 시 `purchases` 생성 후 `carts`에서 제거(트랜잭션). `unique(buyer_id, prompt_id)`로 재구매 방지.
...
- **as-is:**
  - 타이틀: `구매 내역` + 요약(`총 N개 프롬프트 · 결제 금액`) + `[전체 영수증 다운로드]`.
  - 통계 카드 3개: 구매한 프롬프트 / 총 결제 금액 / 총 주문 수.
  - 검색·상태 필터·정렬.
  - **목록 카드:** 썸네일, 카테고리 배지, 제목, 작성자, 구매일, 가격, `[다운로드]`·`[리뷰]`.
  - 리뷰는 다이얼로그에서 별점·내용 작성/수정.
  - **비어 있음:** `아직 구매한 프롬프트가 없습니다.` + `[프롬프트 둘러보기]`.
- **to-be:**
  - DB 조인 목록(가격/결제수단 등 확장 가능).
  - 서명 URL 파일 다운로드, `reviews` 테이블 + RLS.
...
- `string[]` (prompt id). 중복 불가, 수량 없음. 이미 구매한 id는 담기 불가.
...
- `{ id: string; date: string }[]` (`date` = `YYYY-MM-DD`). 동일 id 재구매 불가.

#### Reviews

- `Record<promptId, { rating: number; content: string; updatedAt: string }>` (구매자 인메모리).
...
- `id`, `created_at`, `user_id`, `prompt_id`
- `unique(user_id, prompt_id)`, 수량 필드 없음
...
- **장바구니:** 동일 id 중복 담기 금지(토스트 안내). 이미 구매한 상품 재구매·재담기 불가. 결제(시뮬레이션) 성공 시 해당 id를 purchases에 추가하고 cart에서 제거.
- **리뷰/다운로드:** 구매 내역에서 본문 `.txt` 다운로드 및 별점·리뷰 작성(인메모리).
...
| 영역 | as-is | to-be |
|------|--------|--------|
| 상품 소스 | `lib/data.ts` 시드 12개 | Supabase `prompts` |
